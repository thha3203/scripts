-- loadstring(game:HttpGet("https://raw.githubusercontent.com/thha3203/scripts/refs/heads/main/pixel-blade-v5.lua"))()
...
do
    ...
    -- Closest mob finder
    local function closest_mob(is_raid)
        ...
        for _, v in next, workspace:GetChildren() do
            ...
            if v ~= local_player.Character 
                ...
                and (has_hadEntrance or is_raid)
            then
            ...
            elseif v:FindFirstChild("worm") and v.worm:FindFirstChild("Health") and v.worm.Health.Value > 0 and (has_hadEntrance or is_raid) then
                local dist = (v:GetPivot().Position - local_player.Character:GetPivot().Position).Magnitude
                if dist < closest_distance then
                    closest_distance = dist
                    closest_mob = v
...
-- Auto Farm toggle
    ...
    local transdelay = 1.5
    local raid_room_position = Vector3.new(-749.591552734375, -194.07485961914062, -232.5177459716797)

    local Toggle4 = Tabs.Main:AddToggle("AutoRaids", {
        Title = "Auto Raids (Kill Aura)",
        Description = "Use this for kill aura version without swing.",
        Default = false
    })
    ...
    local Toggle5 = Tabs.Main:AddToggle("AutoPickUpgrade", {
        Title = "Auto Pick Upgrades",
        Default = false
    })

    Toggle5:OnChanged(function()
        if Options.AutoPickUpgrade.Value then
            task.spawn(function()
                while Options.AutoPickUpgrade.Value do
    ...
    Options.AutoPickUpgrade:SetValue(false)
    ...
    -- Auto Anti-Afk
    local Toggle6 = Tabs.AntiAfk:AddToggle("AntiAfk", {
        Title = "Anti-Afk", 
